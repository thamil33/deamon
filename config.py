...
# These are now derived from PRIMARY/REASONING settings for backward compatibility
LLM_API_PROVIDER = PRIMARY_PROVIDER  # For existing code that uses this variable
EVOLUTION_MODEL = REASONING_MODEL   # Evolution uses reasoning model
RITUAL_MODEL = REASONING_MODEL      # Rituals use reasoning model
...
if LLM_API_PROVIDER == "openrouter" and not OPENROUTER_API_KEY:
    raise ValueError("LLM_API_PROVIDER is set to 'openrouter' but OPENROUTER_API_KEY is not set in .env")
if LLM_API_PROVIDER == "lmstudio" and not LM_STUDIO_API_KEY:
    print("WARNING: LM_STUDIO_API_KEY is not set. Using default 'lm-studio'.")
if LLM_API_PROVIDER == "lmproxy" and not LMPROXY_BASE_URL:
